...
There was another factor in choosing our data access layer - team size. Relay - a javascript framework for accessing data - for example, would be more appropriate for a larger team working at scale, but the F8 app was being developed by one person, with a few others in support for design. This had a big influence on the type of data access method we chose in the F8 app.
